  ...
  if (sheet) {
    return (
      <div className="flex items-center gap-2 p-3 bg-gray-800 border border-gray-700 rounded-lg">
        <FileText size={16} className="text-gray-100 shrink-0" />
        <span className="text-sm text-gray-100 flex-1 truncate">{sheet.filename}</span>
        <Button
          variant="ghost"
          size="sm"
  ...
  return (
    <div
      ...
      className={`flex flex-col items-center justify-center gap-2 p-6 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
        dragging ? 'border-gray-200 bg-gray-100/5' : 'border-gray-700 hover:border-gray-600'
      }`}
    >
      <Upload size={24} className="text-gray-400/70" />
      <p className="text-sm text-gray-400">{saving ? 'Saving…' : 'Upload project sheet (PDF, image)'}</p>
      <input
        ref={fileRef}
        type="file"
        accept=".pdf,image/*"
