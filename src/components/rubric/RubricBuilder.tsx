...
import { ChevronDown, ChevronUp, Trash2, ArrowUp, ArrowDown, BookmarkPlus, LayoutTemplate, BookOpen, ChevronRight, Sparkles } from 'lucide-react'
import { GuidedRubricBuilder } from './GuidedRubricBuilder'
...
function CriterionRow({
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-md shadow-black/30">
      <div className="flex items-center gap-2 px-3 py-2.5">
        <div className="flex flex-col gap-0.5 shrink-0">
          <button
            onClick={onMoveUp}
            disabled={index === 0}
            className="p-0.5 text-gray-400/70 hover:text-gray-100 disabled:opacity-30 disabled:cursor-not-allowed rounded"
          >
            <ArrowUp size={12} />
          </button>
          <button
        ...
        <span className="flex-1 text-sm font-medium text-gray-100 truncate min-w-0">
          {c.name || <span className="text-gray-400 italic">Criterion {index + 1}</span>}
        </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-1.5 shrink-0">
            <input
              ...
              onBlur={save}
              className="w-14 text-xs text-center bg-gray-900 border border-gray-700 rounded-lg px-2 py-1 text-gray-100 focus:outline-none focus:border-gray-200-muted"
            />
          ...
          <div key={c.id} className="bg-gray-800 border border-gray-700 rounded-lg">
            <div className="px-3 py-2 flex items-center justify-between bg-gray-750 border-b border-gray-700 rounded-t-lg">
              <span className="text-sm font-medium text-gray-100">
                {c.name || <span className="italic text-gray-400">Unnamed criterion</span>}
              </span>
...
export function RubricBuilder({ projectId, criteria, projectName }: Props) {
  ...
  if (criteria.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center p-8 min-h-0">
        <div className="bg-gray-800 border border-gray-700 rounded-xl p-8 max-w-sm w-full flex flex-col gap-4 items-center text-center">
          <div className="w-12 h-12 rounded-full bg-gray-100/10 flex items-center justify-center">
            <BookOpen size={22} className="text-gray-100" />
          </div>
          <div>
          ...
          >
            ...
            <p className="text-xs text-gray-400 leading-relaxed pl-5">
              Answer a few questions about your project and I'll generate criteria and descriptors for you.
            </p>
          </button>
          ...
          {/* Manual — how many criteria */}
          <div className="w-full flex flex-col gap-3">
            <p className="text-sm text-gray-400">Start blank — how many criteria?</p>
            <div className="flex items-center justify-center gap-4">
              <button
                onClick={() => setCriteriaCount(v => Math.max(1, v - 1))}
                className="w-9 h-9 rounded-lg bg-gray-750 border border-gray-700 text-gray-100 hover:bg-gray-700 flex items-center justify-center text-lg font-bold"
              >−</button>
              <span className="text-3xl font-bold text-gray-100 w-8 text-center tabular-nums">{criteriaCount}</span>
              <button
                onClick={() => setCriteriaCount(v => Math.min(12, v + 1))}
                className="w-9 h-9 rounded-lg bg-gray-750 border border-gray-700 text-gray-100 hover:bg-gray-700 flex items-center justify-center text-lg font-bold"
              >+</button>
            </div>
            <Button variant="primary" className="w-full justify-center" disabled={starting} onClick={handleStart}>
              {starting ? 'Creating…' : 'Start building'}
            </Button>
            <Button variant="ghost" className="w-full justify-center" onClick={() => setTemplatePickerOpen(true)}>
  ...
  return (
    <div className="flex flex-1 min-h-0 overflow-hidden">
      ...
      <div className="w-1/2 flex flex-col border-r border-gray-700 overflow-hidden">
        ...
        {/* Step tabs — pill segmented control */}
        <div className="shrink-0 px-4 py-3 border-b border-gray-700">
          <div className="bg-gray-900 rounded-xl p-1 flex gap-1">
            <button
              onClick={() => setStep(1)}
              className={cn(
                'flex-1 py-1.5 text-xs font-medium transition-all flex items-center justify-center gap-1.5 rounded-lg',
                step === 1 ? 'bg-gray-800 text-gray-100 shadow-sm' : 'text-gray-400 hover:text-gray-200'
              )}
            >
              <span className={cn(
            ...
            <button
              ...
              className={cn(
                'flex-1 py-1.5 text-xs font-medium transition-all flex items-center justify-center gap-1.5 rounded-lg',
                step === 2 ? 'bg-gray-800 text-gray-100 shadow-sm' : 'text-gray-400 hover:text-gray-200'
              )}
            >
              <span className={cn(
                'w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0',
        ...
        {step === 1 && (
          <>
            ...
            {/* Weight banner */}
            <div className={cn(
              'shrink-0 px-5 py-3 border-t border-gray-700 flex items-center gap-3',
              ...
              {!weightOk && (
                <span className="text-xs text-gray-400">
                  {totalWeightPct < 100
                    ? `— ${100 - totalWeightPct}% unallocated · grades will be wrong until this is fixed`
                    : `— ${totalWeightPct - 100}% over 100% · grades will be wrong until this is fixed`}
                </span>
              )}
            ...
            {/* Action bar */}
            <div className="shrink-0 px-5 py-3 border-t border-gray-700 flex items-center gap-2">
              {savingTemplate ? (
                <div className="flex items-center gap-2 flex-1">
                  <input
                    ...
                    className="flex-1 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-100 placeholder-chiffon-muted/50 focus:outline-none focus:border-gray-200-muted"
                  />
                ...
                <>
                  ...
                  <div className="flex-1" />
                  <Button size="sm" variant="ghost" onClick={() => setStep(2)} className="text-gray-100 hover:text-gray-100">
      ...
      <div className="h-full overflow-y-auto p-4">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-700 shadow-md shadow-black/30">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-800 border-b border-gray-700">
                <th className="text-left px-4 py-3 text-gray-400 font-medium">Criterion</th>
                <th className="text-left px-4 py-3 text-gray-400 font-medium">Description</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium whitespace-nowrap">Marks</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Wt</th>
                {hasDescriptors && LEVELS.map(l => (
                  <th key={l.id} className="text-left px-4 py-3 font-medium whitespace-nowrap">
            ...
            <tbody>
              {criteria.map((c, i) => {
                ...
                return (
                  <tr key={c.id} className={cn(i % 2 === 0 ? 'bg-gray-900' : 'bg-gray-850', 'border-b border-gray-700')}>
                    <td className="px-4 py-3 text-gray-100 font-medium align-top">
                      {c.name || <span className="text-gray-400 italic">Criterion {i + 1}</span>}
                    </td>
                    <td className="px-4 py-3 text-gray-400 align-top">{c.description}</td>
                    <td className="px-4 py-3 text-gray-100 text-right align-top tabular-nums">{c.maxMarks}</td>
                    <td className="px-4 py-3 text-gray-100 text-right align-top tabular-nums">
                    ...
                    {hasDescriptors && LEVELS.map(l => {
                      ...
                      return (
                        <td key={l.id} className="px-4 py-3 align-top">
                          {d?.text ? (
                            <div>
                              <span className={cn('text-xs font-semibold tabular-nums', l.textColor)}>
                                {pts}pts
                              </span>
                              <p className="text-gray-400 mt-1 text-xs leading-snug">{d.text}</p>
                            </div>
                          ) : (
                            <span className={cn('text-xs tabular-nums', l.textColor)}>{pts}pts</span>
                          )}
                        </td>
                      )
                    })}
            ...
            <tfoot>
              <tr className="bg-gray-800 border-t border-gray-700">
                <td colSpan={2} className="px-4 py-3 text-gray-400 font-medium">Total</td>
                <td className="px-4 py-3 text-gray-100 font-semibold text-right tabular-nums">
                  {criteria.reduce((s, c) => s + c.maxMarks, 0)}
                </td>
                <td className={cn(
                  'px-4 py-3 font-semibold text-right tabular-nums',
