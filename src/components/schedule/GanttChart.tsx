...
export function GanttChart({ projects, classStartDate, className = 'Class' }: Props) {
  ...
  // ── Build B&W print SVG (separate from the screen SVG) ──────────────────
  function buildPrintSvg(): SVGSVGElement {
    ...
    for (const t of monthTicks) {
      ...
      svg.appendChild(txt(t.label, {
        x: t.x + 8, y: 30,
        fill: '#222222', 'font-size': 17,
    ...
    // Header labels
    svg.appendChild(txt('PROJECT', {
      x: 16, y: 36, fill: '#333333', 'font-size': 15,
      'font-family': 'ui-sans-serif, system-ui, sans-serif',
      'font-weight': 800, 'letter-spacing': '0.08em',
    }))
    svg.appendChild(txt(`${fmtDay(rangeStart)} → ${fmtDay(rangeEnd)}`, {
      x: 16, y: 62, fill: '#777777', 'font-size': 13,
    ...
    dated.forEach((_, i) => {
      if (i % 2 !== 0) {
        svg.appendChild(el('rect', { x: 0, y: rowTopY(i), width: SVG_W, height: ROW_H, fill: '#f8f8f8' }))
      }
      svg.appendChild(el('line', {
        x1: 0, y1: rowTopY(i) + ROW_H, x2: SVG_W, y2: rowTopY(i) + ROW_H,
    ...
    if (todayX !== null) {
      svg.appendChild(el('line', {
        ...
        stroke: '#111111', 'stroke-width': 2, 'stroke-dasharray': '7 4',
      }))
    ...
    dated.forEach((p, i) => {
      ...
      if (startDate) {
        ...
        g.appendChild(txt(label, {
          x: LABEL_W - 14, y: cy + 7,
          fill: labelFill, 'font-size': 18,
          'font-family': 'system-ui, sans-serif', 'font-weight': labelWeight,
          'text-anchor': 'end', opacity,
        }))

        const bar = el('rect', {
          x: startX, y: cy - BAR_H / 2, width: barW, height: BAR_H,
          rx: 4, fill, stroke: '#444444', 'stroke-width': 1, opacity,
        })
        g.appendChild(bar)

        if (barW > 120) {
          g.appendChild(txt(`${fmtDay(startDate)} – ${fmtDay(dueDate)}`, {
            x: startX + barW / 2, y: cy + 6,
            fill: '#111111', 'font-size': 14,
        ...
        g.appendChild(el('line', {
          x1: LABEL_W + 8, y1: cy, x2: dueX - ds - 4, y2: cy,
          stroke: '#bbbbbb', 'stroke-width': 1, 'stroke-dasharray': '5 4',
        }))

        const diamond = el('polygon', {
          points: `${dueX},${cy - ds} ${dueX + ds},${cy} ${dueX},${cy + ds} ${dueX - ds},${cy}`,
          fill: '#444444', stroke: '#222222', 'stroke-width': 1, opacity,
        })
        g.appendChild(diamond)

        g.appendChild(txt(fmtDay(dueDate), {
          x: dueX + ds + 8, y: cy + 7,
          fill: '#333333', 'font-size': 15,
          'font-family': 'ui-monospace, monospace', 'font-weight': 700, opacity,
        }))
      }
    ...
    // Legend
    const legendX = LABEL_W + 12
    const legendY = totalH - 12
    svg.appendChild(el('line', {
      x1: legendX, y1: legendY, x2: legendX + 28, y2: legendY,
      stroke: '#111111', 'stroke-width': 2, 'stroke-dasharray': '7 4',
    }))
    svg.appendChild(txt('Today', {
      x: legendX + 36, y: legendY + 5,
      fill: '#555555', 'font-size': 13, 'font-family': 'system-ui, sans-serif',
    }))
    if (hasAnyStart) {
      svg.appendChild(el('rect', {
        x: legendX + 106, y: legendY - 8, width: 16, height: 16,
        rx: 3, fill: '#aaaaaa', stroke: '#444444', 'stroke-width': 1,
      }))
      svg.appendChild(txt('Project bar', {
        x: legendX + 128, y: legendY + 5,
        fill: '#555555', 'font-size': 13, 'font-family': 'system-ui, sans-serif',
      }))
    } else {
      const px = legendX + 120, py = legendY
      svg.appendChild(el('polygon', {
        points: `${px},${py - 10} ${px + 10},${py} ${px},${py + 10} ${px - 10},${py}`,
        fill: '#444444', stroke: '#222222', 'stroke-width': 1,
      }))
      svg.appendChild(txt('Due date — add start date for full bars', {
        x: legendX + 136, y: legendY + 5,
        fill: '#555555', 'font-size': 13, 'font-family': 'system-ui, sans-serif',
      }))
    }
  ...
  function exportPdf() {
    ...
    printStyle.textContent = `
      @media print {
        ...
        #__gantt_print_root,
        #__gantt_print_root * { visibility: visible !important; }
      }
    `
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* SVG chart */}
      <div className="rounded-xl border border-gray-700 overflow-hidden shadow-lg shadow-black/40">
        <svg
          ref={svgRef}
          viewBox={`0 0 ${SVG_W} ${totalH}`}
          width="100%"
        ...
        >
          ...
          {/* Backgrounds */}
          <rect width={SVG_W} height={totalH} fill="#18181b" />
          <rect width={SVG_W} height={HEADER_H} fill="#1f1f22" />

          {/* Month tick lines */}
          {monthTicks.map((t, i) => (
            <g key={i}>
              <line x1={t.x} y1={36} x2={t.x} y2={totalH} stroke="#3f3f46" strokeWidth="1" />
              <text x={t.x + 8} y={28} fill="#9ca3af" fontSize="15"
                fontFamily="ui-monospace, monospace" fontWeight="600">{t.label}</text>
            </g>
          ))}

          {/* Header borders */}
          <line x1={0} y1={HEADER_H} x2={SVG_W} y2={HEADER_H} stroke="#3f3f46" strokeWidth="1" />
          <line x1={LABEL_W} y1={0} x2={LABEL_W} y2={totalH} stroke="#3f3f46" strokeWidth="1" />

          {/* Header labels */}
          <text x={16} y={36} fill="#6b7280" fontSize="13"
            fontFamily="ui-sans-serif, system-ui, sans-serif" fontWeight="700" letterSpacing="0.08em">
            PROJECT
          </text>
          <text x={16} y={60} fill="#4b5563" fontSize="13" fontFamily="ui-monospace, monospace">
          ...
          {dated.map((p, i) => (
            <g key={p.id + '-row'}>
              {i % 2 !== 0 && (
                <rect x={0} y={rowTopY(i)} width={SVG_W} height={ROW_H} fill="rgba(255,255,255,0.018)" />
              )}
          ...
          {dated.map((p, i) => {
            ...
            const label     = p.name.length > 28 ? p.name.slice(0, 27) + '…' : p.name
            const labelFill   = isPast ? '#6b7280' : isActive ? '#f3f4f6' : '#d1d5db'
              ...
              return (
                <g key={p.id}>
                  ...
                  <rect x={startX} y={cy - BAR_H / 2} width={5} height={BAR_H}
                    rx="3" fill="rgba(255,255,255,0.30)" opacity={opacity} />
                  {barW > 120 && (
                  ...
                  <circle cx={dueX} cy={cy} r="6" fill="#fff" opacity={opacity * 0.8} />
                </g>
              )
            } else {
              const ds = 13
          ...
          <g transform={`translate(${LABEL_W + 12}, ${totalH - 12})`}>
            ...
            {hasAnyStart ? (
              <>
                ...
                <text x={120} y={5} fill="#6b7280" fontSize="13" fontFamily="system-ui, sans-serif">Due date</text>
              </>
            ) : (
              <>
                <polygon points="120,-10 130,0 120,10 110,0" fill="#9b9b9b" opacity={0.6} />
                <text x={136} y={5} fill="#6b7280" fontSize="13" fontFamily="system-ui, sans-serif">Due date — add start date for full bars</text>
              </>
            )}
          </g>
        </svg>
