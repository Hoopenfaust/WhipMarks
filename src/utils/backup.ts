...
import type { Class, Student, Project, RubricCriterion, Mark, RubricDescriptor, RubricTemplate } from '../types'
...
export async function saveToLocalStorage() {
  const [classes, students, projects, criteria, marks, descriptors, rubricTemplates] = await Promise.all([
    db.classes.toArray(),
    db.students.toArray(),
    db.projects.toArray(),
    db.criteria.toArray(),
  ...
  const payload: BackupPayload = { v: 1, ts: Date.now(), classes, students, projects, criteria, marks, descriptors, rubricTemplates }
  try {
...
export async function downloadBackup() {
  const [classes, students, projects, criteria, marks, sheets, descriptors, rubricTemplates] = await Promise.all([
    db.classes.toArray(),
    db.students.toArray(),
    db.projects.toArray(),
    db.criteria.toArray(),
...
async function applyPayload(payload: BackupPayload, includeSheets = false) {
  await db.transaction('rw', [db.classes, db.students, db.projects, db.criteria, db.marks, db.projectSheets, db.descriptors, db.rubricTemplates], async () => {
